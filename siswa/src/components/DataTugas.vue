...
methods:{
  ...
  async postUpload(){
    ...
    form.append("penugasan_id", this.tugas_id)
    try {
      await axios.post("tugasSiswa/upload",form)
      this.$toast.success('Tugas berhasil diupload')
      this.file=null
      this.getUpload()
    } catch (error) {
       console.error(error)
       let msg = 'Gagal upload tugas'
       if (error.response) {
  ...
  getDownloadUrl(filePath) {
    ...
    return `${process.env.VUE_APP_STORAGE_URL || 'http://127.0.0.1:8000/storage/'}${filePath}`;
  },
