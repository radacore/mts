<template>
    <div>
      ...
      <q-card style="width: 700px; max-width: 80vw;">
        ...
        <template v-slot:body-cell-aksi="props">
          <q-td :props="props">
            <save-jumlah-alat-2 :id="props.row.jpid" :diberi="props.row.diberi"/>
          </q-td>
        </template>
        </q-table>
        </q-card-section>
  ...
  async getKatalog(){
      this.loading=true
      await axios.get("filterTopikAlat/"+this.katalog_id+"/"+this.paid).then((response)=>{
